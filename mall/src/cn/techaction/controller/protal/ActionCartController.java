...
public class ActionCartController {
	...
	public SverResponse<String> addProductCart(HttpSession session,Integer productId,Integer count){
		...
		if(user == null){
			return SverResponse.createByErrorMessage("请登录后在查看购物车！");
		}
	 ...
	 * @return
	 */
	@RequestMapping(value="/findcheckedcarts.do")
	@ResponseBody
	public SverResponse<ActionCartVo> findCheckedCarts(HttpSession session){
		User user = (User) session.getAttribute(ConstUtil.CUR_USER);
		if(user == null){
			return SverResponse.createByErrorMessage("请登录后在查看购物车！");
		}
		return aCartService.findCheckedCarts(user.getId());
	}
	/**
	 * 清空购物车
	 * @param session
