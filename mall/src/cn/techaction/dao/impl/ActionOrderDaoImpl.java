...
public class ActionOrderDaoImpl implements ActionOrderDao {
	...
	public int getTotalRecord(Integer uid, Integer status) {
		//System.out.println("dao: " + uid + "    " + status);
	...
	public int insertOrder(ActionOrder order) {
		String sql ="insert into action_orders(id,order_no,uid,addr_id,amount,type,freight,"
				+ "status,payment_time,delivery_time,finish_time,close_time,updated,created)"
				+ " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		Object[] params ={order.getId(),order.getOrderNo(),order.getUid(),order.getAddrId(),order.getAmount(),
				order.getType(),order.getFreight(),order.getStatus(),order.getPaymentTime(),
				order.getDeliveryTime(),order.getFinishTime(),order.getCloseTime(),order.getUpdated(),
				order.getCreated()};
		try {
	...
	public List<ActionOrder> searchOrders(Long orderNo) {
		...
		try{
			if(orderNo!=null){
				...
				sql=sql+" and order_no like ? order by created";
