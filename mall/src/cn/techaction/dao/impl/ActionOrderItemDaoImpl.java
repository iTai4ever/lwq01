...
public class ActionOrderItemDaoImpl implements ActionOrderItemDao {
	...
	public List<ActionOrderItem> getItemByOrderNo(Long orderNo) {
		...
		String sql ="select "+this.alias+" from action_order_items where order_no = ? order by created";	
		try {
	...
	public int[] batchInsert(List<ActionOrderItem> orderItems) {
		...
		for(int i = 0;i<orderItems.size();i++){
			...
			params[i]= new Object[]{
					item.getUid(),item.getOrderNo(),item.getGoodsId(),item.getGoodsName(),item.getIconUrl(),
					item.getPrice(),item.getQuantity(),item.getTotalPrice(),item.getCreated(),item.getUpdated()
			};
		}
		try {
