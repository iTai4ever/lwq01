...
public class ActionAddressDaoImpl implements ActionAddressDao {
	...
	public int insertAddress(ActionAddress addr) {
		// TODO Auto-generated method stub
		String sql = "insert action_address(user_id,name,phone,mobile,province,city,district,addr,zip,default_addr,created,updated) values(?,?,?,?,?,?,?,?,?,?,?,?)";
		Object[] params ={
				addr.getUid(),
				addr.getName(),
				addr.getPhone(),
...
//		if(addr.getCreated()!=null){
//			sql+=" ,created = ?";
//			params.add(addr.getCreated());
//		}
//		if(addr.getUpdated()!=null){
//			sql+=" ,updated = ?";
//			params.add(addr.getUpdated());
//		}
	...
	public List<ActionAddress> findAddrByUserId(Integer userId) {
		// TODO Auto-generated method stub
		String sql ="select id,user_id as uid,name,phone,mobile,province,city,district,addr,zip,default_addr,del_state,created,updated from action_address where user_id = ? and del_state = 0";
				//+ "order by default_addr,updated desc ";
		try {
	...
	public ActionAddress findDefaultAddr(Integer userId) {
		// TODO Auto-generated method stub
		String sql =  "select id,user_id as uid,name,phone,mobile,province,city,district,addr,zip,default_addr,del_state,created,updated"
				+ " from action_address where user_id = ? and default_addr = 1 and del_state = 0 ";
		try {
